...
def func_deltas(buses, gens, lines, V, slack_b, slack_b_g, baseMVA=100):
    ...
    #jad p_gens = tf.tensor_scatter_nd_add(new_gens, slack_gens, pg_slack)
    #slack_in_gens_expanded = tf.expand_dims(slack_in_gens, 0)
    #p_gens = tf.tensor_scatter_nd_add(new_gens, slack_in_gens_expanded, pg_slack)

    #pg_slack_adjusted = tf.squeeze(pg_slack, axis = 0)
    #p_gens = tf.tensor_scatter_nd_add(new_gens, slack_in_gens, pg_slack_adjusted)
    indice_slack_gens = tf.constant([[slack_b_g]])
    p_gens = tf.tensor_scatter_nd_add(new_gens, indice_slack_gens, pg_slack)
    #pg_slack_adjusted = tf.squeeze(pg_slack, axis=0)
    #p_gens = tf.tensor_scatter_nd_add(new_gens, slack_in_gens, pg_slack_adjusted)
