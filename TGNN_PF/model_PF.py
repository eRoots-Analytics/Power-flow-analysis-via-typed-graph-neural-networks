...
class TGN_PF(object):
    ...
    def update_V(self, deltaPV, deltaPQ, PV_nodes, PQ_nodes, V_batch):
        ...
        #jad
        indices_dPVang = tf.cast(tf.reshape(PV_nodes, [-1, 1]), dtype=tf.int64)
        updates_dPVang = tf.squeeze(deltaPV * self.scaling_factor)
        shape_dPVang = total_nodes

        dPVang = tf.scatter_nd(indices_dPVang,
                               updates_dPVang,
                               shape_dPVang)

        #jad
        indices_dPQang = tf.cast(tf.reshape(PQ_nodes, [-1, 1]), dtype=tf.int64)
        #updates_dPQang = tf.cast(tf.squeeze(tf.slice(deltaPQ * self.scaling_factor, [0, 0], [deltaPQ.shape[0], 1])), dtype=tf.int64)
        updates_dPQang = tf.squeeze(tf.slice(deltaPQ * self.scaling_factor, [0, 0], [deltaPQ.shape[0], 1]))
        shape_dPQang = total_nodes

        dPQang = tf.scatter_nd( indices_dPQang,
                               updates_dPQang,
                               shape_dPQang)

        #jad
        indices_dPQmag = tf.cast(tf.reshape(PQ_nodes, [-1, 1]), dtype=tf.int64)
        updates_dPQmag = tf.squeeze(tf.slice(deltaPQ * self.scaling_factor, [0, 1], [deltaPQ.shape[0], 1]))
        shape_dPQmag = total_nodes

        dPQmag = tf.scatter_nd( indices_dPQmag,
                               updates_dPQmag,
                               shape_dPQmag)
